...
### Completed (player skill — `replay.initData` / MMR, 0.7.0)

- [x] **`replay.initData` decoding** (`init_data.rs`): `decode_init_data(&[u8]) -> InitData` walks `SInitData.m_syncLobbyState.m_userInitialData` (the 16-entry lobby slot array) and extracts each slot's `m_name` and `m_scaledRating`. `Replay` now carries an `init_data` field, and — the part most consumers want — `Player` carries `mmr: Option<i64>`, joined from the lobby slot via the newly decoded `SPlayerListEntry.m_workingSetSlotId`. This is the only place in a replay where **player skill** is recorded at all; it was added to unblock `sc2trainer`'s amateur-side threshold calibration, which needs to stratify a replay corpus by skill.
  - **`replay.initData` is bit-packed, not versioned** — contrary to what its sibling metadata stream `replay.details` would suggest. Blizzard's own `decode_replay_initdata` builds a `BitPackedDecoder`; verified empirically before any code was written (a real extracted stream starts with `0x10`, the 5-bit array count, not the `0x05` struct tag every versioned stream opens with). The practical consequence is that there is **no `skip_value` escape hatch**: every one of `SLobbyUserInitialData`'s 19 fields has to be read at exactly the right width or `m_scaledRating` — the *last* field — silently decodes as garbage. The whole struct is therefore modelled even though only two fields are kept. Field widths were transcribed from Blizzard's `s2protocol` typeinfos and cross-checked as identical across builds 95248 / 97425 / 97563.
  - **Invalid ratings are excluded, never fabricated.** SC2 writes a `-36400` sentinel in some replays (documented in Blizzard's own issue tracker); that and anything outside a loose plausibility range (`1..=20000`, whose only job is to catch decode drift) become `None`. There is deliberately no accessor for the raw value — a caller that could see it would eventually average it.
  - **Real-corpus verification (593 replays, 0 load failures, 1186 player-sides).** 424 sides carry an MMR; every one of them lands in `4303..=6912`, with **zero** `-36400` sentinels and **zero** implausible values — a misaligned decode would produce values in the millions, so the complete absence of garbage is itself the strongest evidence the widths are right. Named players rank as they should (Clem 6912 > MaxPax 6785 > Lambo 6620). The remaining 762 sides have a genuinely *absent* optional rather than a bad value: **tournament/custom lobbies do not record ladder MMR at all**, which is why the entire 350-side HomeStory Cup batch honestly reports `None`.
...
│   ├── init_data.rs      # replay.initData decoding (SInitData -> lobby slots + MMR)
